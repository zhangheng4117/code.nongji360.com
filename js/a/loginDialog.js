...
(function(){
	...
	var cssStr = '<style type="text/css">\
		...
		.login_l{float:left;width:180px; margin-left:50px;text-align:center;overflow:hidden;}\
		.login_l img:hover{margin-left:-186px;}\
		.close_icon{position:absolute;right:0;top:0;}\
		.login_r{text-align:center;float:right;width:348px;margin-top:20px;border-left:1px solid #eaeaea;}\
		.login_form{width:260px;margin:0px auto;}\
		.account{margin-top:15px;}\
		.account input{width:255px;height:40px;line-height:40px;border:1px solid #d0d0d0;border-radius:5px;font-size:16px;text-indent:5px;}\
		.account div{margin:2px 0px 0px 5px;text-align:left;color:#f00;font-size:12px;}\
		.find_pw{width:265px;text-align:right;margin:0 auto;line-height:30px;}\
		.login_button{font-size:18px;text-align:left;width:265px;margin:0 auto;}\
		.login_button input{background-color:#ec600b;width:80px;height:40px;line-height:40px;border:0;font-size:18px;color:#FFF;margin-right:10px;}\
		#authid.placeholder{background:url('+currentServer+'/images/text_bg.gif) no-repeat 0 0;}\
		#password.placeholder{background:url('+currentServer+'/images/text_pwd.gif) no-repeat 0 0;}\
		</style>';
	...
	/*if ( !RegEx.ip(currentServer) )
	{
		if ( ''==getCookie('ASPSESSIONIDSARTCQQD') && ''!=getCookie('NJSESSID') )
		{
			setCookie('ASPSESSIONIDSARTCQQD', getCookie('NJSESSID'), 0, '/', currentServer.substr(currentServer.indexOf('.')+1));
		}
	}*/
...
function ajaxLogin(fn)
{
	...
	$.ajax({
		...
		'data':{'username':authid,'password':password},
		'processData':true,
		'type':'get',
	...
	/*
	$.post(HTTP_WWW+'/user/ajax_logok.asp', {'username':authid,'password':password}, function(data, status){
		if ( 'success'!=status )
		{
			alert("网络连接失败，请重新登录");
			return false;
