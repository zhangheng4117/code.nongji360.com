(function( $ ){
    ...
    $(function() {
        ...
        uploader = WebUploader.create({
            ...
            server: HTTP_VIDEO_FILE+'/oUpload.php?r='+checkTime(d.getHours())+ checkTime(d.getMinutes()) + checkTime(d.getSeconds())+rand,
			formData:{uid:123},
            ...
            if ( state === 'ready' ) {
                ...
                if ( stats.uploadFailNum ) {
                    text = '已成功上传' + stats.successNum+ '个视频，'+
                        stats.uploadFailNum + '个视频上传失败，<a class="retry" href="#">重新上传</a>失败视频'
                }
        ...
        uploader.onUploadSuccess = function(file,data){
            $.each(data, function(index,e){
                ...
                if(data[index] == 'error')
                {
                    $('<p class="error">上传失败，请重试</p>').appendTo($("#"+file.id));
